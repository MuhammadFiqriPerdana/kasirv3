...
<table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
  ...
  <tbody>
    <?php
    $no = 1;
    $data_barang = mysqli_query($conn, "SELECT * FROM produk WHERE toko = '" . $_SESSION['toko'] . "' ORDER BY idproduk ASC");
            ...
            <form method="post">
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <input type="text" placeholder="0" name="Edit_Tanggal_Kadaluarsa" value="<?php echo $d['stok']; ?>" class="form-control" required>
                </div>
              </div>
...
if (isset($_POST['TambahProduk'])) {
  ...
  $InputProduk = mysqli_query($conn, "INSERT INTO produk (kode_produk,nama_produk,harga_modal,harga_jual,tgl_input, tgl_kadaluarsa, stok, toko) values ('$kodeproduk','$namaproduk','$harga_modal','$harga_jual','$tanggal_input','$tanggal_kadaluarsa', '$stok', '" . $_SESSION['toko'] . "')");
  if ($InputProduk) {
...
if (isset($_POST['SimpanEdit'])) {
  ...
  if ($HasilData > 0) {
    ...
    $cekDataUpdate =  mysqli_query($conn, "UPDATE produk SET kode_produk='$kodeproduk1',
        nama_produk='$namaproduk1',harga_modal='$harga_modal1',harga_jual='$harga_jual1', tgl_input='$tanggal_input1', tgl_kadaluarsa='$tanggal_kadaluarsa1', stok='$stok1' WHERE idproduk='$idproduk1'") or die(mysqli_connect_error());
    if ($cekDataUpdate) {
      echo '<script>history.go(-1);</script>';
    } else {
    ...
    <div class="modal-content border-0">
      <form method="post">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="samll">Tanggal Kadaluarsa :</label>
            <input type="date" name="Tambah_Tanggal_Kadaluarsa" class="form-control" required>
          </div>
          <div class="form-group">
            <label class="samll">Stok :</label>
            <input type="tex" name="Tambah_Stok" class="form-control" required>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
