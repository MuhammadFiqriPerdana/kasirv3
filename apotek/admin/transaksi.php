...
<table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
  ...
  <tbody>
    <?php
    $no = 1;
    $data_laporan = mysqli_query($conn, "SELECT * FROM inv WHERE toko = '" . $_SESSION['toko'] . "' AND status='selesai' ORDER BY invid ASC");
    ...
    ?>
      <tr>
        ...
        <td>Rp.<?php
                $result2 = mysqli_query($conn, "SELECT SUM(subtotal) AS count1 FROM laporan WHERE invoice='$oninv' WHERE toko = '".$_SESSION['toko']."'");
